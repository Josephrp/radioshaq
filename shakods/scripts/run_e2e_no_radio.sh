...
# Usage: from shakods: ./scripts/run_e2e_no_radio.sh
set -e
...
export BASE_URL
export SHAKODS_RADIO__ENABLED=false
export SHAKODS_RADIO__AUDIO_INPUT_ENABLED=false
export RADIO_ENABLED=false

command -v pm2 >/dev/null 2>&1 || { echo "PM2 not found. Install: npm i -g pm2"; exit 1; }

pm2 delete shakods-api 2>/dev/null || true
sleep 1

echo "Starting shakods-api (test env, no radio), port ${TEST_PORT}..."
