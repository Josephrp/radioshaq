...
from shakods.config.schema import AudioConfig, ResponseMode, TriggerMatchMode
from shakods.specialized.radio_rx_audio import (
    ConfirmationManager,
...
def test_trigger_filter_confidence_reject() -> None:
    ...
    f = TriggerFilter(config)
    assert f.check("shakods here", 0.5) is False


def test_trigger_filter_contains() -> None:
    config = AudioConfig(
        trigger_enabled=True,
        trigger_phrases=["shakods"],
        trigger_match_mode=TriggerMatchMode.CONTAINS,
    )
    f = TriggerFilter(config)
    assert f.check("this is shakods calling", 0.8) is True
